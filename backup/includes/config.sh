...
if [ ! -f $ROOT/$CONFIG_FILE ]; then
    ...
    echo "<?xml version="1.0"?><config><mysql><hostname>127.0.0.1</hostname><port>3306</port><username>acore</username><password>acore</password></mysql><backup><location>/opt/backup</location><max_files>24</max_files></backup></config>" | xmllint --format - > $ROOT/$CONFIG_FILE
    exit 1
fi
...
LOCATION="$(echo "cat /config/backup/location/text()" | xmllint --nocdata --shell $ROOT/$CONFIG_FILE | sed '1d;$d')"
