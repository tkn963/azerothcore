...
if [ ! -f $CONFIG_FILE ]; then
    ...
    echo "<?xml version=\"1.0\"?><config><mysql><hostname>127.0.0.1</hostname><port>3306</port><username>acore</username><password>acore</password><database><auth>acore_auth</auth><characters>acore_characters</characters><world>acore_world</world></database></mysql><core><directory>/opt/azerothcore</directory></core><world><name>AzerothCore</name><motd>Welcome to AzerothCore!</motd><id>1</id><ip>127.0.0.1</ip><game_type>1</game_type><realm_zone>8</realm_zone><player_limit>1000</player_limit><skip_cinematics>0</skip_cinematics><start_level>1</start_level><start_money>0</start_money><always_max_skill>0</always_max_skill><all_flight_paths>0</all_flight_paths><maps_explored>0</maps_explored><allow_commands>0</allow_commands><quest_ignore_raid>1</quest_ignore_raid><prevent_afk_logout>0</prevent_afk_logout><rates><experience>1</experience><rested_exp>1</rested_exp><reputation>1</reputation><money>1</money><crafting>1</crafting><gathering>1</gathering><weapon_skill>1</weapon_skill><defense_skill>1</defense_skill></rates><gm><login_state>1</login_state><visible>0</visible><chat>0</chat><whisper>0</whisper><gm_list>0</gm_list><who_list>0</who_list><allow_invite>0</allow_invite><lower_security>0</lower_security></gm></world><module><ahbot><enabled>false</enabled><enable_seller>0</enable_seller><enable_buyer>0</enable_buyer><account_id>1</account_id><character_guid>1</character_guid><min_items>250</min_items><max_items>250</max_items></ahbot><skip_dk_area><enabled>false</enabled></skip_dk_area><kickstarter><enabled>false</enabled><functions><equipment>true</equipment><gems>true</gems><glyphs>true</glyphs><spells>true</spells><proficiency><enabled>true</enabled><max_skill>true</max_skill></proficiency><mounts>true</mounts><utilities><enabled>true</enabled><name_change>true</name_change><race_change>true</race_change><faction_change>true</faction_change><appearance_change>true</appearance_change></utilities></functions></kickstarter><experienced><enabled>false</enabled></experienced></module></config>" | xmllint --format - > $CONFIG_FILE
    exit 1
fi
...
WORLD_IP="$(echo "cat /config/world/ip/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"
...
MODULE_SKIP_DK_AREA_ENABLED="$(echo "cat /config/module/skip_dk_area/enabled/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"

MODULE_KICKSTARTER_ENABLED="$(echo "cat /config/module/kickstarter/enabled/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"
MODULE_KICKSTARTER_FUNCTIONS_EQUIPMENT="$(echo "cat /config/module/kickstarter/functions/equipment/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"
MODULE_KICKSTARTER_FUNCTIONS_GEMS="$(echo "cat /config/module/kickstarter/functions/gems/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"
MODULE_KICKSTARTER_FUNCTIONS_GLYPHS="$(echo "cat /config/module/kickstarter/functions/glyphs/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"
MODULE_KICKSTARTER_FUNCTIONS_SPELLS="$(echo "cat /config/module/kickstarter/functions/spells/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"
MODULE_KICKSTARTER_FUNCTIONS_PROFICIENCY_ENABLED="$(echo "cat /config/module/kickstarter/functions/proficiency/enabled/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"
MODULE_KICKSTARTER_FUNCTIONS_PROFICIENCY_MAX_SKILL="$(echo "cat /config/module/kickstarter/functions/proficiency/max_skill/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"
MODULE_KICKSTARTER_FUNCTIONS_MOUNTS="$(echo "cat /config/module/kickstarter/functions/mounts/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"
MODULE_KICKSTARTER_FUNCTIONS_UTILITIES_ENABLED="$(echo "cat /config/module/kickstarter/functions/utilities/enabled/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"
MODULE_KICKSTARTER_FUNCTIONS_UTILITIES_NAME_CHANGE="$(echo "cat /config/module/kickstarter/functions/utilities/name_change/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"
MODULE_KICKSTARTER_FUNCTIONS_UTILITIES_RACE_CHANGE="$(echo "cat /config/module/kickstarter/functions/utilities/race_change/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"
MODULE_KICKSTARTER_FUNCTIONS_UTILITIES_FACTION_CHANGE="$(echo "cat /config/module/kickstarter/functions/utilities/faction_change/text()" | xmllint --nocdata --shell $CONFIG_FILE | sed '1d;$d')"
