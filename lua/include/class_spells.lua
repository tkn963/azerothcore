CLASS_SPELL_LIST = {
    -- Warrior
    {{78,1},{2457,1},{6673,1},{100,4},{772,4},{6343,6},{34428,6},{284,8},{1715,8},{71,10},{355,10},{2687,10},{6546,10},{7386,10},{72,12},{5242,12},{7384,12},{1160,14},{6572,14},{285,16},{694,16},{2565,16},{676,18},{8198,18},{845,20},{6547,20},{12678,20},{20230,20},{5246,22},{6192,22},{1608,24},{5308,24},{6190,24},{6574,24},{1161,26},{6178,26},{871,28},{8204,28},{1464,30},{2458,30},{6548,30},{7369,30},{20252,30},{11549,32},{11564,32},{18499,32},{20658,32},{7379,34},{11554,34},{1680,36},{6552,38},{8205,38},{8820,38},{11565,40},{11572,40},{11608,40},{20660,40},{23922,40},{11550,42},{11555,44},{11600,44},{11578,46},{11604,46},{11566,48},{11580,48},{20661,48},{23923,48},{1719,50},{11573,50},{11609,50},{11551,52},{11556,54},{11601,54},{11605,54},{23924,54},{11567,56},{20662,56},{11581,58},{11574,60},{20569,60},{23925,60},{25286,60},{25288,60},{25289,60},{25241,61},{25202,62},{25269,63},{23920,64},{25234,65},{25258,66},{29707,66},{25264,67},{469,68},{25208,68},{25231,68},{2048,69},{25242,69},{3411,70},{25203,70},{25236,70},{30324,70},{30356,70},{30357,70},{46845,71},{64382,71},{47449,72},{47519,72},{47470,73},{47501,73},{47439,74},{47474,74},{47487,75},{55694,75},{47450,76},{47465,76},{47520,77},{47436,78},{47502,78},{47437,79},{47475,79},{47440,80},{47471,80},{47488,80},{57755,80},{57823,80}},
    -- Paladin
    {{0, 0}},
    -- Hunter
    {{0, 0}},
    -- Rogue
    {{0, 0}},
    -- Priest
    {{0, 0}},
    -- Death Knight
    {{0, 0}},
    -- Shaman
    {{0, 0}},
    -- Mage
    {{0, 0}},
    -- Warlock
    {{0, 0}},
    -- Empty
    {{0, 0}},
    -- Druid
    {{0, 0}}
}

CLASS_TALENT_LIST = {
    -- Warrior
    {{21551,48,12294},{21552,54,21551},{21553,60,21552},{25248,66,21553},{30330,70,25248},{47485,75,30330},{47486,80,47485},{30016,60,20243},{30022,70,30016},{47497,75,30022},{47498,80,47497}},
    -- Paladin
    {{0, 0, 0}},
    -- Hunter
    {{0, 0, 0}},
    -- Rogue
    {{0, 0, 0}},
    -- Priest
    {{0, 0, 0}},
    -- Death Knight
    {{0, 0, 0}},
    -- Shaman
    {{0, 0, 0}},
    -- Mage
    {{0, 0, 0}},
    -- Warlock
    {{0, 0, 0}},
    -- Empty
    {{0, 0, 0}},
    -- Druid
    {{0, 0, 0}}
}

CLASS_PROFICIENCY_LIST = {
    -- Warrior
    {{SPELL_ONE_HANDED_AXES,0},{SPELL_TWO_HANDED_AXES,0},{SPELL_ONE_HANDED_MACES,0},{SPELL_TWO_HANDED_MACES,0},{SPELL_POLEARMS,0},{SPELL_ONE_HANDED_SWORDS,0},{SPELL_TWO_HANDED_SWORDS,0},{SPELL_STAVES,0},{SPELL_BOWS,0},{SPELL_GUNS,0},{SPELL_DAGGERS,0},{SPELL_CROSSBOWS,0},{SPELL_MAIL,0},{SPELL_FIST_WEAPONS,0},{SPELL_PARRY,6},{SPELL_DUAL_WIELD,20},{SPELL_PLATE_MAIL,40}},
    -- Paladin
    {{0, 0}},
    -- Hunter
    {{0, 0}},
    -- Rogue
    {{0, 0}},
    -- Priest
    {{0, 0}},
    -- Death Knight
    {{0, 0}},
    -- Shaman
    {{0, 0}},
    -- Mage
    {{0, 0}},
    -- Warlock
    {{0, 0}},
    -- Empty
    {{0, 0}},
    -- Druid
    {{0, 0}}
}
